...
static void _3XNN()
{
    uint16_t x = (op & 0x0F00) >> 8, nn = op & 0x00FF;
    if (v[x] == nn)
        pc += 0x0002;
}
static void _4XNN()
...
static void _6XNN()
{
    uint16_t nn = op & 0x00FF, x = (op & 0x0F00) >> 8;
    v[x] = nn;
}
static void _7XNN()
{
...
{
    uint16_t x = (op & 0x0F00) >> 8, y = (op & 0x00F0) >> 4;
    uint8_t flag = v[x] + v[y] > 0xFF ? 0x01 : 0x00;
    v[x] = v[x] + v[y];
    v[0xF] = flag;
}
static void _8XY5()
{
    uint16_t x = (op & 0x0F00) >> 8, y = (op & 0x00F0) >> 4;
    uint8_t flag = v[x] - v[y] < 0x00 ? 0x00 : 0x01;
    v[x] = v[x] - v[y];
    v[0xF] = flag;
}
static void _8XY7()
{
    uint16_t x = (op & 0x0F00) >> 8, y = (op & 0x00F0) >> 4;
    uint8_t flag = v[y] - v[x] < 0x00 ? 0x00 : 0x01;
    v[x] = v[y] - v[x];
    v[0xF] = flag;
}
static void _8XY6()
...
int main(int argc, char *argv[])
{
    ...
    SDL_SetWindowTitle(window, "Chip8 Emu-1.0");
    ...
    while (SDL_TRUE)
    {
        ...
        char fpsLabel[256];
        sprintf(fpsLabel, "Current FPS: %f, Average FPS: %f, Current Perf: %ld",
                1.0f / frameTime,
